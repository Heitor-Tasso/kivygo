            ...
            if not self.editor.ordered:
                for pol in itervalues(self.editor.rec[self.editor.frame]):
                    if pol['number'] > self.pol['number']:
                        pol['number'] -= 1
                self.pol['number'] = len(
                    self.editor.rec[self.editor.frame]) - 1
...
# ,,,------------------------ EDITOR -----------------------
    ...
    def pick_point(self, point, *args):
        ...
        self.to_transfer.append(
            (self.pol, point.pol['btn_points'].index(point)))
    ...
    def transfer_points(self, point=None, pp=None):
        ...
        if point:
            ...
            pol['btn_points'][self.index].state = 'down'
            msg = 'New polygon ({}) of {} points'.format(
                self.pol, len(ordered))
        self.empty_cut()
        self.save_state(msg)
        self.draw()
    ...
    def on_key(self, window, key, *args):
        ...
        if key == 273:  # up
            for point in self.to_transfer:
                curr_point = self.rec[self.frame][point[0]
                                                  ]['btn_points'][point[1]]
                if ['ctrl'] in args:
...
# ,,,--------------------- IMAGE-TRACING -----------------------
    ...
    def make_border(self, contour, *args):
        ...
        self.complexity = max(
            int(round(sum([len(shp) for shp in shapes]) * .03)), 3)
    ...
    def link_points(line, x, y, found, *args):
        ...
        while [x, y] not in border:
            ...
            downright = [x + 1, y - 1] if x + \
                1 < len(line) and y > 0 else [x, y]
            down = [x, y - 1] if y > 0 else [x, y]
            downleft = [x - 1, y - 1] if x > 0 and y > 0 else [x, y]
            left = [x - 1, y] if x > 0 else [x, y]
            upleft = [x - 1, y + 1] if x > 0 and y + \
                1 < len(line[0]) else [x, y]
                ...
                if len(pattern) == 2:  # if a pattern already exists,
                    ...
                    if pattern[0][0] == previous_angle and pattern[0][1] >= count:
                        ...
                        if pattern[0][1] > count:
                            simple_border.append(
                                border[temp[0][0] + (count - 1)])

                    # If repetition stops, the first element's candidate is kept
                    else:
                        simple_border.append(temp[0][1])

                    # first element removed (so second is first)
                    pattern.pop(0)
                    temp.pop(0)
                # complete element added
                pattern.append([previous_angle, count])
                # new element's first point as a candidate
                temp.append([i, pt])
                previous_angle = angle  # establishing new element's angle
...
class LoadDialog(BoxLayout):
    ...
    def create_drives(self):
        ...
        # add empty space under the drives
        self.ids.drives_list.add_widget(Label())
