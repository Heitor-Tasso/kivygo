...
class Rotabox(Widget):
    ...
    def set_draw(self):
        ...
        if self.anim:
            pols = max([len(frame)
                        for frame in self.custom_bounds.values()])
            length = max([len(pol)
                          for pol in self.custom_bounds.values()])
        else:
        ...
        for i in range(pols):
            if i not in self.open_bounds or not self.segment_mode:
                self.draw_lines += tuple([Line(close=True,
                                         dash_offset=3, dash_length=5)])
            else:
                self.draw_lines += tuple([Line(close=False,
                                         dash_offset=3, dash_length=5)])
