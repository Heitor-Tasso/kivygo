...
class CircularProgressBar(Widget):
	...
	def set_normalised_progress(self, norm_progress):
		...
		self._progress = ceil(self.min_progress + (norm_progress - self.normalised_min) * dif1 / dif2)


	value_normalized = AliasProperty(get_normalised_progress, set_normalised_progress, bind=["_progress"])
	...
	def on_max_progress(self, *args):
		if self.max_progress <= self.min_progress:
			raise ValueError(f"Maximum progress - {self.max_progress} - must be greater than minimum progress ({self.min_progress})!")
	...
	def on_min_progress(self, *args):
		if self.min_progress > self.max_progress:
			raise ValueError(f"Minimum progress - {self.min_progress} - must be smaller than maximum progress ({self.max_progress})!")
