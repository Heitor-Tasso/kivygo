...
class _Example(kivygoApp):
    ...
    def animate(self, dt):
        ...
        if bar.progress < bar.max_progress:
            ...
            Clock.schedule_once(lambda *a: Clock.schedule_interval(self.animate, 0.01), 1)
    ...
    def start_animation_first(self, *args):
        ...
        anim.bind(on_complete=lambda *a: Clock.schedule_once(self.start_animation_first, 1))
        anim.start(bar)
    ...
    def start_animation_second(self, *args):
        ...
        bar.progress = bar.min_progress
        anim = Animation(progress=bar.max_progress, duration=6, transition="out_quad")
        anim.bind(on_complete=lambda *a: Clock.schedule_once(self.start_animation_second, 1))
        anim.start(bar)
